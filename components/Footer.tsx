...
export const Footer: React.FC = () => {
  ...
  const openModal = (type: 'terms' | 'refund' | 'privacy') => {
    setModalType(type);
  };
        ...
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="mt-12 flex flex-col md:flex-row justify-between items-center text-gray-500 text-sm">
            ...
            <div className="flex gap-6 mt-4 md:mt-0">
              <button onClick={() => openModal('terms')} className="hover:text-white transition-colors">이용약관</button>
              <button onClick={() => openModal('refund')} className="hover:text-white transition-colors">환불정책</button>
              <button onClick={() => openModal('privacy')} className="hover:text-white transition-colors">개인정보처리방침</button>
            </div>
          </div>
        </div>
      </footer>
